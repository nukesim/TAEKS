...
export default function TakeCard({take,onBack,onFade,onShare,compact=false}){
  return <article className={`receipt ${compact?'compact':''}`}>
    <div className="receipt-top"><div className="brandmark"><span className="brand-fire"><Flame size={18} fill="currentColor"/></span>TAEKS</div><span className={`status ${take.status==='HIT'?'hit':take.status==='MISS'?'miss':''}`}>{take.status||'LIVE'}</span></div>
    <div className="eyebrow">{take.category} · {take.season}</div>
    <h2>{take.canonicalText}</h2>
    <div className="author"><div className="avatar">{(take.username||'B')[0].toUpperCase()}</div><div><strong>@{take.username||'bryson'}</strong><span>LOCKED {new Date(take.stampedAt).toLocaleString([], {month:'short',day:'numeric',year:'numeric',hour:'numeric',minute:'2-digit'}).toUpperCase()}</span></div></div>
    ...
    <div className="receipt-footer"><span><LockKeyhole size={14}/> TAEKS #{take.id}</span><span>HASH {take.hash}</span></div>
